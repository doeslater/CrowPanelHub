#!/usr/bin/env bash
# Compiles basic_board/pwr_pin and basic_board/rgb_led with extra compiler
...
{
  echo "--- basic_board/pwr_pin ---"
  arduino-cli compile --warnings all --fqbn "$FQBN" "$SCRIPT_DIR/basic_board/pwr_pin/"
  echo
  echo "--- basic_board/rgb_led ---"
  arduino-cli compile --warnings all --fqbn "$FQBN" "$SCRIPT_DIR/basic_board/rgb_led/"
} > "$LOG_FILE" 2>&1
