...
//
// The Android sender doesn't exist yet -- whatever it ends up doing must
// produce these exact values, or update both sides together.
...
// RTC/NTP of its own, so the sender must supply the current time (UTC).
const size_t TIMESTAMP_SIZE = 4;   // uint32_t, unix epoch seconds (UTC)
