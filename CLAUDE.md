...
**Milestone 1**: send a hardcoded checkerboard test pattern from the phone to the ESP32 over USB serial and see it render on the panel, using a full (not partial) e-paper refresh. The pattern is generated programmatically in Kotlin (a small loop building packed 1-bit bytes) — no bundled binary asset — so the code is readable/teachable and so image processing isn't a variable while proving out the USB→panel pipeline. This is fire-and-forget: the ESP32 does not send anything back, and Android considers the send successful once the bytes are written to the serial port. Visual confirmation is by looking at the panel during bring-up. **Partially started**: the firmware half (`receive_image/`) exists, compiles for `esp32:esp32:esp32s3`, and has been flashed to and verified against real hardware (see Architecture below). The Android send path doesn't exist yet.
...
- **Wire protocol** (phone → ESP32 only, one-directional): a minimal length-prefixed binary frame — `[1-byte magic][4-byte little-endian payload length][4-byte little-endian unix epoch seconds (UTC)][payload: packed 1-bit bitmap, 400×300 = 15,000 bytes][1-byte checksum]`. Binary, not text/ASCII, since the payload is already binary. Magic byte is `0xA5`; checksum is an 8-bit sum (mod 256) of the payload bytes only (the timestamp field isn't covered by it). The timestamp exists so the panel can display a "last updated" date/time label — the ESP32 has no RTC and no Wi-Fi/NTP in milestone 1, so it cannot know the time on its own; the sender must supply it (UTC epoch seconds) in every frame. The label is rendered and displayed as UTC, not the phone's local time — no timezone offset field. All of this is implemented firmware-side in `receive_image/` — the Android sender doesn't exist yet, so when it's built it must produce these exact values or both sides need updating together.
...
- **`receive_image/`** (repo root, sibling of `app/` and `display_text/`): the milestone-1 firmware sketch — a serial receive loop, not present in `display_text/`. Compiles for `esp32:esp32:esp32s3` and **has been flashed to and verified against real hardware** (a CrowPanel board on `/dev/ttyUSB0`, CH340 bridge chip) using a hand-written Python script standing in for the not-yet-built Android sender — magic byte, length, timestamp, checksum, and rendering all confirmed working end-to-end; the panel visually showed a correct checkerboard with the date/time label.
  - `receive_image.ino`: scans incoming `Serial` bytes for the magic byte, then reads the length-prefixed frame described in the Wire protocol bullet above (including the UTC timestamp field), verifies the checksum, and renders via `epd.drawBitmap(...)` for the bitmap plus a `strftime`-formatted "last updated" UTC date/time label drawn over the bottom of the frame, then `epd.display()` using the same power-cycle (`epdPower`/`epd.init`/`epd.hibernate`) pattern as `display_text.ino`. Any malformed/mismatched frame is dropped with a `Serial.println` and no response sent — fire-and-forget, matching the milestone-1 spec.
  ...
  - `send_test_frame.py`: the Python script standing in for the Android sender, mentioned above — stdlib-only (no `pyserial`), resets the board via raw `termios`/`fcntl` DTR/RTS control, builds a checkerboard-pattern frame with the current UTC time via `build_frame(payload)`, sends it, and prints the board's `Serial` responses. Exposes `send_payload(payload)` so other scripts can reuse the reset/send/report logic with a different bitmap.
